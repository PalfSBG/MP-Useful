---------------------------------------------------
---Vegas Weekly Traffic by Value Band

--Purpose: 
--Code to pull out vegas visits and logged-in users by value band for the current and previous year.
---------------------------------
...
where a.feature = 'vgs_visits'
and b.feature = 'current_value.vgs.bands'
and cast(a.value as int) > 0
and (a.year = year(now()) OR a.year = year(now())-1) 
)

Select  week_start_date, vgs_vb, count(distinct cust_id) as users,
...
from base
group by vgs_vb, week_start_date
